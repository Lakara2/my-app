...
const SessionManagementPage = () => {
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      try {
        axios
          .get(`${process.env.REACT_APP_API_URL}/users/sessions`)
  ...
  const handleLogoutSession = async (sessionId) => {
    ...
    try {
      axios
        .post(`${process.env.REACT_APP_API_URL}/logout-session`, { sessionId })
      ...
      setError(
        "Erreur lors de la déconnexion de la session. Veuillez réessayer.",
      );
    }
  };
